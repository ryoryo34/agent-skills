...
5. **literal 解釈を前提に書く** — Opus 4.8 / GPT-5.5 世代のモデルは指示を字義通り・正確に守る（Anthropic / OpenAI 両公式ガイダンスの収束点）。生成・追記するルールはすべて次のスタイルで書く:
   - 肯定形 + 理由付き（「〜しない」の羅列より「〜する。理由: …」が効く。モデルは理由から汎化できる）
   - ALWAYS / NEVER / 必ず 等の強制語は真の不変条件（セキュリティ、データ破壊防止、統計ゲート等）だけに使う。判断が要る場面は「条件 → 行動」の decision rule 形式で書く
   - 手順の逐次指定より、ゴール + 制約 + 完了条件（Done when）を書く（逐次手順の過剰指定は両モデルで品質を下げる）
   - 同じ主題のルールを複数ファイル・複数箇所に書かない（矛盾と二重管理の芽。GPT-5.x は矛盾指示への感度が特に高い）
...
- `~/.codex/AGENTS.md`（グローバル）に persistence（完遂までの自律実行 + 停止条件）と進捗報告の指針があるか確認し、無ければ追加を提案する。GPT-5.x は明示がないと早期終了しやすい。プロジェクト側 AGENTS.md には重複させない（グローバルとの二重管理を避ける）

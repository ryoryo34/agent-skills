...
- コード修正の完了報告前に、必ず自分で `{{build-command}}` と `{{test-command}}` を実行する。失敗時は結果をそのまま報告する
- {{UI があるプロジェクトの場合: スクリーンショット等の実行時確認も自分で行ってから報告する}}
...
- {{緊急修正の扱い: hotfix ブランチを使うか、着手前にユーザー確認するか}}
```
